...
本目录实现对拖曳系数 **\(K,B,C\)** 的独立 **Kernel Ridge（RBF 核）** 拟合；超参数 **\(\gamma\)** 由 **留一交叉验证（LOOCV）** 在给定网格上择优；**\[实现时见代码\]** 将拆分为：
...
采用 **`sklearn.kernel_ridge.KernelRidge`**，`kernel='rbf'`：
...
- **`gamma`**：由下节网格 + LOOCV 选取；非 RBF 相关参数不参与。
...
\[
\gamma_i = 1 + i\cdot \mathrm{stride},\quad i=0,1,\ldots,30 \quad\text{（共 31 个候选）}.
\]

对每个 \(\gamma_i\) 与每个目标 \(y\)：

1. LOO：**\(n\)** 折，每折在 **\(n-1\)** 条上训练、在留出的一条上得分。
2. **测试侧 LOOCV MSE**：\(\mathrm{MSE}_{\mathrm{test}}(\gamma)=\frac{1}{n}\sum_j (y_j-\hat y_j^{(-j)})^2\)。
3. **训练侧 LOOCV MSE（折内）**：每折仅在 **\(n-1\)** 条训练点上算拟合误差并平均，再在折上对外层平均——记为 \(\mathrm{MSE}_{\mathrm{train}}(\gamma)\)。

以 **最小的 \(\mathrm{MSE}_{\mathrm{test}}(\gamma)\)** 对应的 \(\gamma^\*\) 为最终 \(\gamma\)（平局取实现规定的第一个）。
...
- 各目标的 **LOOCV 误差表**（如 CSV：**`gamma`、折内平均训练 MSE、折外平均测试 MSE**），用于描 \(\gamma\)–误差曲线。
- **`manifest.json`**（或其它摘要文件）：\(n\) 样本、选用的 **`alpha`**、**`stride`** 与 \(\gamma\) 列表、每个目标 **`best_gamma`**、最佳 LOOCV 测试误差、相对路径文件名等。
...
2. **`errors_by_target`**  
   **结构化**的各个模型、各个 \(\gamma\) 上的误差，便于 UI 或直接绑图。**建议最小结构**示例（实现可等价使用 `dataclass` / `TypedDict`）：

```text
{
  "K": { "gamma": float[], "train_mse": float[], "test_mse": float[] },
  "B": { "gamma": ..., "train_mse": ..., "test_mse": ... },
  "C": { "gamma": ..., "train_mse": ..., "test_mse": ... }
}
```

三者 **`gamma`** 序列相同（由同一 \(\gamma\) 网格决定）；向量长度均为候选个数（31）。
...
# 训练结束时顺带生成 γ～LOOCV 误差图（与 graph.py loocv 等价；可用 -graph）
python kernel_regression/run.py train \
  --data-dir /path/to/training_json_dir \
  --out-dir ./krr_outputs \
  --graph
...
- **`train`**：`--out-dir` 语义上对应 **`model_path`**（父目录）；**stdout** 仅一行输出 **`saved_root`**（`kernel_regression_<timestamp>`），便于脚本解析； **`--graph` / `-graph`**：成功后调用 **`plot_loocv_gamma_curves`**，默认 PNG 与同目录 **`kbc_gamma_loocv_curves.png`**，PNG 绝对路径另以 **`[graph] <path>`** 写入 **stderr**。
...
\(n&lt;2\) 时无法进行 LOOCV，实现应报错并提示。**极少样本下「最优 \(\gamma\)」不确定性大**，仅作工程上的折中选取；增量数据后可再次训练更新时间戳目录。
...
单次训练 artefact 目录内已写有 **`kbc_krr_loocv_K.csv` / `B` / `C`**（列为 `gamma,train_mse_loocv_mean,test_mse_loocv_mean`）。作图方式为 **对每个目标一行子图**，同一子图叠加 **训练 MSE、测试 MSE** ～ **γ**。**未指定 `--out` 时**，PNG **`kbc_gamma_loocv_curves.png`** 与 **`kbc_krr_*.joblib`/CSV 同目录**；**`--out` 为相对路径**则相对于 **`--model-dir`**；**绝对路径**则按其保存。
