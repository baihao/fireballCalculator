...
负责管理建模与预测标签页中的各类图表组件；在用户点击「开始计算」并完成
...
class ModelTabChartController:
    ...
    def __init__(self, ui_builder) -> None:
        ...
        self._velocity_chart: Optional[DiameterVelocityChart] = ui_components.get('velocity_chart')
    ...
    def reset(self) -> None:
        ...
        if self._velocity_chart:
            self._velocity_chart.set_placeholder("计算完成后显示")
    ...
    def update_expansion_velocity(self,
                                  time_ms: Sequence[float],
                                  diameter_m: Sequence[float]) -> None:
        """更新膨胀速度曲线（由直径时序数值求导）。"""
        self._ensure_initialized(self._velocity_chart, 'velocity_chart')
        self._velocity_chart.update_data(time_ms, diameter_m)  # type: ignore[arg-type]
